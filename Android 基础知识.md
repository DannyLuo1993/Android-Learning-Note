...
（4） 局部内部类需要使用方法的局部变量时，必须用final修饰符 - 原因：避免局部内部类对象被返回到外部类外面使用时，访问不到这个局部变量，所以加final将变量的位置挪到方法区。
...
左边是常规new对象操作，右边花括号是类，但是修饰符和名字等被隐匿了。所以变量名就会持有匿名内部类的引用
...
### 4.2.5 Collection 集合框架

Collection继承自Iterable接口，表示可以直接使用foreach遍历。

Collection接口 - List接口实现  - Vector 接口实现 - ArrayList接口实现 - LinkedList接口实现 （Stack接口实现了Vector接口）

Collection接口 - Queue接口实现 - Deque接口实现

Collection接口 - Set接口实现 - Hashset 接口实现 - LinkedHashSet接口实现

Collection接口 - Set接口实现 - Sorted接口实现 - NavigableSet接口实现 - TreeSet接口实现



Map接口 - Hashable接口实现 - Properties接口实现 

Map接口 - HashMap接口实现 - LinkedHashMap接口实现
...
文件夹、目录不同于文件，因为文件夹中可以放文件，或下一级文件夹。
...
```java
public void test02(){
    File dir = new File("D:/Path/Folder/File");
...
public void deleteDir(File file){
    ...
    if(file.isDirectory()){
        ...
        //经过这个foreach循环，File传入的文件夹的所有下一级删除。
        for (File sub: listFiles){
            deleteDir(sub);
        }
    }
...
public void test(){
    ...
    File[] listFiles = dir.listFiles(new FileFilter(){
        ...
        public boolean accept(File pathname){
            return pathname.getName().endWith(".java") && pathname.getName().contains("Test");
...
（1） 对于File对象来说，输入和输出操作只能是针对文件，不能针对目录。
...
Consists static method that return a Path. This Path is generated by coverting a path string or URI

用来创建Path接口的对象

* Path

可用于在文件系统中定位文件的对象。【用路径名表示一个目录或文件】

* Files

此类仅包含对文件，目录或其他类型的文件进行操作的静态方法。

（1）Files.copy(src, dest, Option... option);

（2）delete（Path path） 功能类似File类的delete，不同的地方是当文件不存在等异常存在时，这个方法会报

（3）deleteIfExists（Path path）

（4）move（Path source， Path target， CopyOption...  options）

（5）List《String》 readAllLines（Path path， Charset cs）



## 4.4 回调模式

如果一个方法的参数是接口类型，则在调用该方法时，需要创建并传递一个实现此接口类型的对象。

而方法在运行时会调用到参数对象中所实现的方法（接口中定义的）
...
（3）网络协议： 对速率、传输代码、代码结构、传输控制步骤、出错控制等制定标准。 通俗地讲就是保住如何准确地到达目的地，然后被正确地解析出数据
...
数据链路层：建立逻辑连接、进行硬件地址寻址、差错校验等功能。 将比特组合成字节进而组合成帧，用MAC地址访问介质，错误发现但不纠正
...
表示层：数据的表示、安全、压缩。格式有：JEPG、ASCII、DECOIC、加密格式等

应用层：网络服务于最终用户的一个接口。协议有：HTTP、FTP、SMTP、DNS、TELNET、HTTPS、POP3等
...
面向连接的，可靠的，基于字节流的，适用于大数据量的传输的协议
